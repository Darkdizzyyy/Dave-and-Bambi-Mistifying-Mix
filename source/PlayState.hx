...
class PlayState extends MusicBeatState
{
	...
	var nativelyPlayableCharacters:Array<String> = ['placeholder'];
	...
	override public function create()
	{
		...
		switch (curStage)
		{
			...
			case 'cheater': // FUCKING IDIOT
			{
				var bg:FlxSprite = new FlxSprite(-320, -160).loadGraphic(Paths.image('voids/cheater'));
			...
			case 'Tessattackbg': // bg is a placeholder btw
			{
				var bg:FlxSprite = new FlxSprite(-320, -160).loadGraphic(Paths.image('voids/tessbg'));
				bg.setGraphicSize(Std.int(bg.width * 2.5));
				bg.antialiasing = true;
				bg.scrollFactor.set(0.1, 0.1);
				add(bg);
		...
		if (formoverride == "none" || formoverride == "bf" || formoverride == "bf" || formoverride == SONG.player1)
        {
		...
		var textYPos:Float = healthBarBG.y + 56;
		if (creditsText)
		{
			textYPos = healthBarBG.y + 30;
		}
		// Add Kade Engine watermark
		var kadeEngineWatermark = new FlxText(4, textYPos, 0, SONG.song, 18);
		...
		if (isStoryMode && !seenCutscene)
		{
			switch (daSong)
			{
				...
				case 'imprecation' | 'caught' | 'blitz': //BRAH DIALOGUE
		...
		if(ret != FunkinLua.Function_Stop) {
			...
			startTimer = new FlxTimer().start(Conductor.crochet / 1000 / playbackRate, function(tmr:FlxTimer)
			{
				...
				switch (swagCounter)
				{
					...
					case 4:
						var creditsPopup = new CreditsPopUp(FlxG.width, 200);
	...
	if (curbg != null)
	{
		...
		else
		{
			if (!paused)
			{
				...
				{
					songTime = (songTime + Conductor.songPosition) / 1000;
				...
				if(updateTime) {
					...
					if(ClientPrefs.timeBarType != 'Song Name')
						timeTxt.text = "(" + FlxStringUtil.formatTime(songTime, false) + " / "+ FlxStringUtil.formatTime(songLength / 1000, false) + ")";
				}
			}
	...
	function goodNoteHit(note:Note):Void
	{
		if (!note.wasGoodHit)
		{
			...
			var animToPlay:String = '';
                ...
                {
                    case 0:
                        if(nativelyPlayableCharacters.contains(formoverride.toLowerCase())) {
	...
	override function stepHit()
	{
		...
		switch (SONG.song.toLowerCase())
        {
			case 'caught':
				...
				{
					case 48:
						subtitleManager.addSubtitle("I dont care!", 0.02, 1);
					case 432:
						subtitleManager.addSubtitle("I will BLOCK YOU!", 0.02, 1);
					case 1075:
						subtitleManager.addSubtitle("Thats it, Thats it", 0.03, 0.3, {subtitleSize: 45});
					case 1361:
						subtitleManager.addSubtitle("GO TO FUCKING HELL!", 0.03, 0.3, {subtitleSize: 45});
                }
        }
