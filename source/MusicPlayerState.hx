...
class MusicPlayerState extends MusicBeatState
{
	...
	override function create()
	{
		...
		{
			var songText:Alphabet = new Alphabet(90, 320, songs[i].songName + (songs[i].hasVocals ? "" : ((songs[i].songName != 'tutorial') ? "-Inst" : "")), true);
		...
		healthBarBG = new FlxSprite(0, 50).loadGraphic(Paths.image('musicplayer/healthBarNew'));
	...
	override function update(elapsed:Float)
	{
		...
		var UI_UP_P = controls.UI_UP_P;
		var UI_DOWN_P = controls.UI_DOWN_P;

		var UI_LEFT_P = controls.UI_LEFT_P;
		var UI_RIGHT_P = controls.UI_RIGHT_P;
